...
const FEATURES = [
    {
        icon: 'scan',
        title: 'Unlock Your Scan Results',
        description: 'See your complete facial analysis and personalized recommendations',
    },
    {
        icon: 'book',
...
export default function SubscriptionScreen() {
    ...
    const handleSubscribe = async () => {
        ...
        try {
            ...
            const response = await api.post(`/api/payment/create-checkout-session?plan_id=${selectedPlan}`);
            ...
            if (checkout_url) {
                ...
                if (supported) {
                    ...
                    // After returning from browser, refresh user to check subscription status
                    // This is a simplified flow - in production you'd use deep links
                    setTimeout(async () => {
            ...
            Alert.alert(
                'Payment Error',
                error.response?.data?.detail || 'Failed to start payment. Please try again.'
            );
        } finally {
            setLoading(false);
        }
